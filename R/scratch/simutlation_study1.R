...
#  plotting the expected, theoretical, and epsilon size. Sample size. Small
#  sets.

# Why is breaking at 10,000?
library(ggplot2)
library(gridExtra)

simulation <- function(n, eps, weight_mat, reg_method, test_size = 100,
                       seed = 43421) {
  ...
  for (i in seq_len(ncol(pred_accuracy))) {
    for (j in seq_len(nrow(pred_accuracy))) {
      if (reg_method == "reg_sine") {
        ...
        temp <- data_gen_asym(n[i], weight_mat = weight_mat, resp_sd = eps[j])
        test <- sample(nrow(temp), test_size)
        data <- temp[-test,]
        test <- temp[test, ]
        test_y <- test[, 1]
        ...
        temp <- data_gen_lm(n[i], weight_mat = weight_mat, resp_sd = eps[j])
        test <- sample(nrow(temp), test_size)
        data <- temp[-test,]
        test <- temp[test, ]
        test_y <- test[, 1]
  ...
  colnames(pred_accuracy) <- c("n7500", "n9000", "n10000")
...
sine_accuracy_res <- simulation(n, eps, mat_sine, "reg_sine", test_size = 300)
sine_accuracy <- sine_accuracy_res[[1]]

ggplot(sine_accuracy) +
  geom_point(aes(x = eps, y = n10)) +
...
ggplot(sine_accuracy) +
  ...
  scale_x_continuous(breaks = seq(.1, 1, by = .1), limits = c(0, 1.1))
ggplot(sine_accuracy) +
  geom_point(aes(x = eps, y = n5000)) +
  geom_line(aes(x = eps, y = eps), color = "red") +
  scale_x_continuous(breaks = seq(.1, 1, by = .1), limits = c(0, 1.1))
ggplot(sine_accuracy) +
  geom_point(aes(x = eps, y = n7500)) +
  geom_line(aes(x = eps, y = eps), color = "red") +
  scale_x_continuous(breaks = seq(.1, 1, by = .1), limits = c(0, 1.1))
ggplot(sine_accuracy) +
  geom_point(aes(x = eps, y = n10000)) +
  geom_line(aes(x = eps, y = eps), color = "red") +
  scale_x_continuous(breaks = seq(.1, 1, by = .1), limits = c(0, 1.1))

# Asymptotic Regression
eps <- seq(from = .1, to = 1, by = .1)
n <- c(100, 300, 500, 700, 900) # add in 5000
mat_asym <- matrix(rep(1, 10), nrow = 2, ncol = 5)
asym_accuracy <- simulation2(n, eps, mat_asym, "reg_asym", test_size = 300)

ggplot(asym_accuracy) +
  geom_point(aes(x = eps, y = n10)) +
  geom_line(aes(x = eps, y = eps), color = "red") +
  scale_x_continuous(breaks = seq(.1, 1, by = .1), limits = c(0, 1.1))
ggplot(asym_accuracy) +
...
mat_lm <- c(1, 3, 2, 7, 4)
lm_accuracy <- simulation2(n, eps, mat_lm, "lm", test_size = 300)
