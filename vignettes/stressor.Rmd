...
and fits the initial machine learning models and then displays the initial RMSE 
...
share similar attributes with one another so therefore if we fit on those groups
...
What we notice about this result is we ignored spatial autocorrelation but when 
we compare the 10 fold cross validation with the clustered cross validation we 
see a general improvement in the RMSE values which gives reason to rise that
maybe there is some other factor underlying, i.e. spatial relationships.

In conclusion, with this package you were able to fit multiple machine learning 
models with a few lines of code. This package was also able to perform 10 fold 
cross validation and clustered cross validation from one line of code. Then with
a simple command can return the RMSE values from the predictions. With this 
package you were able to compare multiple machine learning models on a data set 
to see how they compared against each other. This power to be able to explore is
a compliment to the purpose of R.
...
instantiating the virtual environments. Reticulate is a package in R that 
