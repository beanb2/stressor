...
can show that our models fit data if we are close to the theoretical error. In the graphic below the black dots represent the  value given the current epsilon that we are on. The red line represents the expected theoretical error.
...
. The `mlm_lm` is a list object where the first element is a list of
...
`r trunc(lm_score * 100)/100` compared to `r trunc(top_RMSE * 100)/100`
