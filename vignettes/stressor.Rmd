...
various models. Currently RMSE is the only method supported at this time. The 
...
frame of the initial RMSE values and the corresponding models. If you want to
...
## Real Data Example
We want to show how are functions apply to a real data example. We can simulate 
data but it is never quite like real observed data. The purpose of this data set
is to show its use on the functions in this package, specifically the cross 
...
This next code chunk opens the cleaned Boston data set attached to this package and fits the initial machine learning models and then displays the initial RMSE values from the first fit.
```{r eval=FALSE}
data(boston)
...
mlm_boston_rmse
```
Clustered cross validation is subsetting the parameter space into groups that share similar attributes with one another so therefore if we fit on those groups
the other groups should fit similarly across that missing group space.
...
What we notice about this result is we ignored spatial autocorrelation but when we compare the 10 fold cross validation with the clustered cross validation we see a general improvement in the RMSE values which gives reason to rise that
maybe there is some other factor underlying, i.e. spatial relationships.

In conclusion, with this package you were able to fit multiple machine learning models with a few lines of code. This package was also able to perform 10 fold cross validation and clustered cross validation from one line of code. Then with a simple command can return the RMSE values from the predictions. With this package you were able to compare multiple machine learning models on a data set to see how they compared against each other. This power to be able to explore is a compliment to the purpose of R.
...
instantiating the virtual environments. Reticulate is a package in R that handles the connection between R and python. 

If you receive a warning that says
<p style="text-align:center; color:darkred"> "Warning Message: Previous request to use_python() ... will be ignored. It is superseded by request to use_python()"</p> If the second `use_python` command has the matching virtual environment you can ignore this warning and continue with your analysis.
...
If this error appears restart your R session and run the `create_virtualenv()` function again and there should be no problems attaching it after that.
</div>
